...
SC_MODULE(DummyProcessor)
{
    ...
    void doCPUStuff()
    {
        ...
        {
            uint16_t v = readFromBus(SRC_BASE + 2 * i);
            writeToBus(SRC_BASE + i, v);
        }
        std::cout << "[CPU] decimation done (" << OUT_SAMPLES << " samples kept)" << std::endl;

        // extraction
        std::ofstream outFile("filtered_2x.txt");
        for (int i = 0; i < OUT_SAMPLES; i++)
        {
        ...
        std::cout << "[CPU] wrote filtered_2x.txt (" << OUT_SAMPLES << " samples)" << std::endl;
        std::cout << "[CPU] 2x stage complete." << std::endl;
        sc_stop();
    }
};
