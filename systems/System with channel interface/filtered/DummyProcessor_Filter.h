...
SC_MODULE(DummyProcessor)
{
    ...
    void setupInterruptSources()
    {
        ...
        writeToBus(PIC_BASE + 1, 2); // MMA  -> tag 2 (unused here)
    ...
    void doCPUStuff()
    {
        ...
        // ---- Processing phase: 44 chunks, overwrite in place ----
        int pos = 0;
        int chunkNo = 0;
        while (pos < TOTAL_SAMPLES)
        {
            ...
            writeToBus(DMA_BASE + 2, 0);                      // toAddress (unused here)
            ...
            writeToBus(DMA_BASE + 1, 0);                      // fromAddress (unused here)
            writeToBus(DMA_BASE + 2, AUDIO_BASE + pos);       // toAddress: SAME location (overwrite)
        ...
        // ---- Extraction: read the filtered signal back out ----
